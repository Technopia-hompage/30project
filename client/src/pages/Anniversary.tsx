            ...
            <Card className="bg-white/80 backdrop-blur-sm shadow-xl border-2 border-corporate-blue/20">
              <CardContent className="p-8">
                ...
                <div className="relative mb-6">
                  {/* Scroll Container */}
                  <div className="overflow-x-auto photo-carousel-scroll rounded-lg">
                    <div className="flex space-x-4 p-4" style={{ width: 'max-content' }}>
                      {/* Conference Photo */}
                      <div className="flex-shrink-0 w-72 h-48 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-200">
                        <img 
                          src={conferenceImage} 
                      ...
                      {Array.from({ length: 12 }, (_, i) => (
                        <div key={i + 2} className="flex-shrink-0 w-72 h-48 bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg border-2 border-dashed border-slate-300 flex items-center justify-center">
                          <div className="text-center">
                            ...
                            <p className="text-sm text-slate-500">Photo {i + 2}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex justify-center mt-4">
                    <div className="bg-slate-200 rounded-full px-4 py-2 text-sm text-slate-600">
                      <svg className="inline w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16l-4-4m0 0l4-4m-4 4h18" />
                      </svg>
                      {language === 'jp' && '左右にスクロールして写真をご覧ください'}
                      {language === 'ko' && '좌우로 스크롤하여 사진을 보세요'}
                      {language === 'en' && 'Scroll left and right to view photos'}
                      {language === 'zh' && '左右滚动查看照片'}
                ...
                <div className="text-center mb-4">
                  <p className="text-sm text-slate-600 font-medium mb-2">
                    {language === 'jp' && '現在の写真: 2015年8月26日 医療機器セミナー開催'}
                    {language === 'ko' && '현재 사진: 2015년 8월 26일 의료기기 세미나 개최'}
                    {language === 'en' && 'Current photo: Medical Device Seminar held on August 26, 2015'}
                    {language === 'zh' && '当前照片：2015年8月26日举办医疗器械研讨会'}
                  </p>
                  <p className="text-xs text-slate-500">
                    {language === 'jp' && '13枚中 1枚目 | 残り12枚の写真を追加予定'}
                    {language === 'ko' && '13장 중 1장 | 나머지 12장의 사진을 추가 예정'}
                    {language === 'en' && '1 of 13 photos | 12 more photos to be added'}
                    {language === 'zh' && '13张中的第1张 | 将添加其余12张照片'}
                  </p>
                </div>
