...
export function Auto() {
  ...
  // 헬퍼 함수들
  const getWheelById = (id: number) =>
    wheelModels.find((wheel: any) => wheel.id === id);
  const getBrandById = (id: number) =>
    wheelBrands.find((brand: any) => brand.id === id);
  const getWheelsByBrand = (brandId: number) =>
    wheelModels.filter(
      (wheel: any) => wheel.brandId === brandId && wheel.status === "active",
    );
  const getDiscontinuedWheels = () =>
    wheelModels.filter((wheel: any) => wheel.status === "inactive");
  const {
    data: wheelSpecs = [],
  ...
  const getWheelSpecsByWheelId = (wheelId: number) => {
    ...
    console.log(
      "Auto.tsx: Filtering specs for wheelId",
      wheelId,
      "Found:",
      specs.length,
      "specs",
  ...
  // 특징 편집 함수들 - 관리자 권한 확인
  const handleEditFeatures = (wheel: any) => {
    // 관리자 권한 확인
    if (params.admin !== 'true') {
      alert(language === "jp" ? "編集権限がありません" : 
            language === "ko" ? "편집 권한이 없습니다" : 
            language === "en" ? "No edit permission" : 
            "没有编辑权限");
      return;
    }
  ...
  const handleSaveFeatures = async () => {
    ...
    try {
      const response = await fetch(`/api/wheels/models/${editingWheel.id}/features`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          description: editingWheel.description,
          features: editingWheel.features
        }),
      });
      ...
      alert(language === "jp" ? "特徴の更新に失敗しました" : 
            language === "ko" ? "특징 업데이트에 실패했습니다" : 
            language === "en" ? "Failed to update features" : 
            "特征更新失败");
    }
  };
  ...
  if (wheelId) {
    ...
    console.log(
      "Auto.tsx: Current wheel specs count:",
      currentWheelSpecs.length,
    );
    console.log("Auto.tsx: Specs loading state:", specsLoading);
    console.log("Auto.tsx: All wheel specs count:", wheelSpecs.length);
    console.log(
      "Auto.tsx: First few current specs:",
      currentWheelSpecs.slice(0, 3),
    );
                ...
                <div className="bg-slate-50 rounded-lg p-6">
                  ...
                  <div className="text-center py-8">
                    <p className="text-slate-600 mb-2">
                      {language === "jp"
                        ? "このホイールのスペックデータが見つかりません"
                        : language === "ko"
                          ? "이 휠의 스펙 데이터를 찾을 수 없습니다"
                          : language === "en"
                            ? "No specs data found for this wheel"
                            : "未找到此轮毂的规格数据"}
                    </p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {params.admin === 'true' && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleEditFeatures(wheel)}
                ...
                <div className="bg-slate-50 rounded-lg p-4">
                  <p className="text-slate-700 leading-relaxed">
                    {wheel.features?.[language] || wheel.features?.jp || 
                     (language === "jp" ? "高品質アルミニウム合金製、軽量化と強度の両立、JWL・VIA規格適合品" :
                      language === "ko" ? "고품질 알루미늄 합금제, 경량화와 강도의 양립, JWL・VIA 규격 적합품" :
                      language === "en" ? "High-quality aluminum alloy, achieving both weight reduction and strength, JWL・VIA certified" :
                      "高品质铝合金制，轻量化与强度的兼顾，JWL・VIA规格适合品")}
                  </p>
                </div>
              </div>
  ...
  if (brandId) {
    ...
    return (
      <div className="min-h-screen">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-slate-600">
                {language === "jp" &&
                  `${brand.nameJp}シリーズの全ラインナップをご覧ください`}
                {language === "ko" &&
                  `${brand.nameJp} 시리즈의 전체 라인업을 확인하세요`}
                {language === "en" &&
                  `Explore the complete ${brand.name} series lineup`}
              ...
              {wheels.map((wheel: any) => (
                <AnimatedCard key={wheel.id}>
                  <Card
                    ...
                    onClick={() =>
                      (window.location.href = `/${language}/auto/wheel/${wheel.id}`)
                    }
                  >
        ...
        {(() => {
          const discontinuedWheels = wheelModels.filter(
            (wheel: any) => wheel.brandId === brandId && wheel.status === "inactive"
          );
          
          if (discontinuedWheels.length > 0) {
            return (
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold text-red-900 mb-4">
                      {language === "jp" && "生産終了モデル"}
                      {language === "ko" && "생산종료 모델"}
                      {language === "en" && "Discontinued Models"}
                      {language === "zh" && "停产型号"}
                    </h2>
                    <p className="text-xl text-red-700">
                      {language === "jp" &&
                        `${brand.nameJp}シリーズの生産終了モデルです`}
                      {language === "ko" &&
                        `${brand.nameJp} 시리즈의 생산종료 모델입니다`}
                      {language === "en" &&
                        `Discontinued models from ${brand.name} series`}
                      {language === "zh" && `${brand.name}系列的停产型号`}
                    </p>
                  </div>

                  <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {discontinuedWheels.map((wheel: any) => (
                      <AnimatedCard key={wheel.id}>
                        <Card className="h-full hover:shadow-xl transition-all duration-300 cursor-pointer group bg-red-50 border-red-200">
                          <CardContent className="p-0">
                            ...
                            <div className="p-6">
                              ...
                              <p className="text-red-600 text-sm line-clamp-2 mb-4">
                                {wheel.description[language] || wheel.description.jp}
                              </p>
        ...
        <section className="py-16 lg:py-24 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-slate-600">
                {language === "jp" &&
                  `${brand.nameJp}シリーズの全スペックをご覧ください`}
                {language === "ko" &&
                  `${brand.nameJp} 시리즈의 전체 스펙을 확인하세요`}
                {language === "en" &&
                  `View all specifications for ${brand.name} series`}
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16 lg:py-24 corporate-gradient">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <p className="text-xl text-blue-100 max-w-3xl mx-auto">
              {language === "jp" &&
                "高品質なアルミニウムホイールのラインナップをご覧ください"}
              {language === "ko" && "고품질 알루미늄 휠 라인업을 확인하세요"}
              {language === "en" &&
                "Explore our lineup of high-quality aluminum wheels"}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="text-center">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          "/images/auto/2024-25TECHNOPIA_SELECTION.pdf",
                          "_blank",
                        )
                      }
                    >
                ...
                <CardContent className="p-6">
                  ...
                  <div className="text-center">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          "/images/auto/2022-23TECHNOPIA_SELECTION.pdf",
                          "_blank",
                        )
                      }
                    >
                ...
                <CardContent className="p-6">
                  ...
                  <div className="text-center">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          "/images/auto/2020-21TECHNOPIA_SELECTION.pdf",
                          "_blank",
                        )
                      }
                    >
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-slate-600">
              {language === "jp" &&
                "各ブランドの特徴とラインナップをご覧ください"}
              {language === "ko" && "각 브랜드의 특징과 라인업을 확인하세요"}
              {language === "en" &&
                "Explore the features and lineups of each brand"}
              ...
              .map((brand: any) => {
                ...
                const getBrandLogo = (brandName: string) => {
                  const logoMap: { [key: string]: string } = {
                    CHARITES: "/images/auto/charites_logo.png",
                ...
                return (
                  <AnimatedCard key={brand.id}>
                    <Card
                      className="h-full hover:shadow-xl transition-all duration-300 cursor-pointer group"
                      onClick={() =>
                        (window.location.href = `/${language}/auto/brand/${brand.id}`)
                      }
                    >
                      <CardContent className="p-0">
      ...
      <section className="py-16 lg:py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-slate-600">
              {language === "jp" &&
                "過去に製造されたホイールのアーカイブをご覧ください"}
              {language === "ko" && "과거에 제조된 휠의 아카이브를 확인하세요"}
              {language === "en" &&
                "Browse the archive of previously manufactured wheels"}
      ...
      {/* Features Edit Modal - 관리자만 접근 가능 */}
      {isEditingFeatures && editingWheel && params.admin === 'true' && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold">
                {language === "jp" ? `${editingWheel.nameJp} の特徴を編集` : 
                 language === "ko" ? `${editingWheel.nameJp} 특징 편집` : 
                 language === "en" ? `Edit Features for ${editingWheel.nameJp}` : 
                 `编辑 ${editingWheel.nameJp} 的特征`}
              </h2>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-4">
                {language === "jp" ? "特徴編集 (技術仕様)" : 
                 ...
                 language === "en" ? "Edit Features (Technical Specs)" : 
                 "编辑特征 (技术规格)"}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-sm font-medium text-gray-700">
                    {language === "jp" ? "特徴 (日本語)" : 
                     language === "ko" ? "특징 (일본어)" : 
                     language === "en" ? "Features (Japanese)" : 
                     "特征 (日语)"}
                  </label>
                  <Textarea
                    value={editingWheel.features.jp}
                    onChange={(e) => setEditingWheel({
                      ...editingWheel,
                      features: { 
                        ...editingWheel.features,
                        jp: e.target.value
                      }
                    })}
                    className="mt-2"
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">
                    {language === "jp" ? "特徴 (韓国語)" : 
                     language === "ko" ? "특징 (한국어)" : 
                     language === "en" ? "Features (Korean)" : 
                     "特征 (韩语)"}
                  </label>
                  <Textarea
                    value={editingWheel.features.ko}
                    onChange={(e) => setEditingWheel({
                      ...editingWheel,
                      features: { 
                        ...editingWheel.features,
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">
                    {language === "jp" ? "特徴 (英語)" : 
                     language === "ko" ? "특징 (영어)" : 
                     language === "en" ? "Features (English)" : 
                     "特征 (英语)"}
                  </label>
                  <Textarea
                    value={editingWheel.features.en}
                    onChange={(e) => setEditingWheel({
                      ...editingWheel,
                      features: { 
                        ...editingWheel.features,
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">
                    {language === "jp" ? "特徴 (中国語)" : 
                     language === "ko" ? "특징 (중국어)" : 
                     language === "en" ? "Features (Chinese)" : 
                     "特征 (中文)"}
                  </label>
                  <Textarea
                    value={editingWheel.features.zh}
                    onChange={(e) => setEditingWheel({
                      ...editingWheel,
                      features: { 
                        ...editingWheel.features,
